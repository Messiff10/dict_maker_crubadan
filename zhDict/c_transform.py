    ...
    def __m_translate(self, infile, outfile, mode):
        ...
        with open(infile, 'r', encoding="utf8") as infile:
            ...
            for line in infile.readlines():
                ...
                a = words.translate((self.__ponctuation_ch2en))

                a = self.__m_t(a, mode)
                result = words + '\t' + a + '\t' + str(freq)
                outfile_.write(result.strip())
                outfile_.write('\n')
                if (i % 1000 == 0):
                    outfile_.flush()
        outfile_.close()
...
if __name__ == '__main__':
    input = "/Users/ff/Desktop/chinese2cangjie/zh-words.txt"
    output = "/Users/ff/Desktop/chinese2cangjie/zh-words_change.txt"
    map = "/Users/ff/Desktop/chinese2cangjie/char2cangjie.json"
    mode = 0
    ...
    assert os.path.isfile(input), 'input must be a readable file'
    assert mode in [0, 1], "mode (0: from chinese character to cangjie; 1: from cangjie to chinese character)"
    c_trans = c_transform()

    if mode == 0:
        outfile = c_trans.chinese2cangjie(infile=input, outfile=output, mapfile=map)
    else:
        outfile = c_trans.cangjie2chinese(infile=input, outfile=output, mapfile=map)
