...
def sentense2data(sentense_file, output_datafile, wordmap_all):
    linenum = 0
    with open(sentense_file, "r") as sentense_file_in:
        with open(output_datafile, 'w') as data_file_out:
            ids = 0
            count = 0
            for sentence in sentense_file_in:
                words = regex.split(sentence.strip())
                if len(words) > 1:
                    newwords = '|||'.join(words)  # 拼接单词
                    allletterslist = ''
                    for word in words:
                        count += 1
                        letterslist = ''
                    ...
                    linenum += 1
                    outputline = allletterslist[allletterslist.index('|||') + 3:] + "|#|" + newwords + '\n'
                    # print(outputline)
...
if __name__ == "__main__":
    # language = "ms_MY"
    file_path = "/Users/ff/Desktop/train_data/vi/vi_user_web_case_token.txt"
    # file_path = sys.argv[1]
    # map = sys.argv[2]
    map = "/Users/ff/Desktop/train_data/vi/word_map_sort"
    names = []
