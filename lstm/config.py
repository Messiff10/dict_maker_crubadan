...
import numpy as np
import os
from lstm.data_utility import DataUtility

flags = tf.app.flags.FLAGS
logging = tf.logging

flags.DEFINE_string("model_config", "static-sanity-check.cfg", "Model hyperparameters configuration.")
flags.DEFINE_string("data_path", "/home/pubsrv/data/train_data/en_GB/train_data_en_GB_user_web_mapletters/", "Where the training/test data is stored.")
flags.DEFINE_string("vocab_path", "/home/pubsrv/data/train_data/en_GB/train_data_en_GB_user_web_mapletters/", "Where the training/test data is stored.")
flags.DEFINE_string("file_name_stem", "ptb", "corpus file name (without suffix).")
flags.DEFINE_string("save_path", "lstmmodel/", "Model output directory.")
flags.DEFINE_string("model_name", "lstm_train", "Pick a name for your model.")
...
class Config():
    ...
    def get_config(self, vocab_path, config_filename=None):
        ...
        vocab_file_out = os.path.join(vocab_path, "vocab_in_words")
