...
class LSTMModel(object):
    ...
    def __init__(self, is_training, past=None):
        ...
        self.END_WITH_SYMBOL_REGEX_DEFAULT = re.compile("([a-zA-Z']+)([^a-zA-Z']+$)")
    ...
    def predictnext(self, kc_output_idx, logfile, curr_id, j, temp_y, curr):
        predict_top10 = []
        predict_top10_id = []
        if len(curr[7:]) >= 2 and curr[7:].isupper() and j > 1:  # 输入起始位和第一位字母 并且目标值前两个字母是大写
            # print("temp_y",temp_y)
            ...
            if len(curr[7:]) > 0 and temp_y[:1].isupper():
                for predict in [id for id in kc_output_idx[-1]]:
                    predict_words = self.id2token_out.get(predict, "<unk>")
                    if len(predict_top10) < 10:
                        if predict_words in ["<eos>", "<und>", "<unk>"]:
                            ...
                            predict_top10.append(predict_words.title())
                            predict_top10_id.append(str(predict))
                    else:
                        break
            else:
    ...
    def testlite_lstm(self, sess, lm_tflitepath, kc_tflitepath, data, vocab_path):  ###读取文件
        ...
        count = 0
        save_count = 0
        if len(re.findall(emoji.get_emoji_regexp(), sentence.strip())) == 0:
            word_count = len(sentence.strip())
        else:
            word_count = len(sentence.strip()) + len(re.findall(emoji.get_emoji_regexp(), sentence.strip()))
        ...
        for i in range((length)):
            ...
            temp_y_deal = re.sub(self.END_WITH_SYMBOL_REGEX_DEFAULT, lambda x: x.group(1),
                                 temp_y.strip())

            curr_id = self.word2id_in(lm_data_x[i], return_word=True)  # 输入转id  按照vocab_in_words Faith
            target_id = self.word2id_out(temp_y, return_word=True)  # 预测词转id  忽略大小写
            ...
            for j in range(len(letters_ids)):
                ...
                # curr += temp_y[j]
                if j > 0 and temp_y[j - 1].isupper():
                    # print("upppppp")
                    curr += str(self.id2token_in_letters.get(input_letter, '<unk>')).upper()
                else:
                    curr += str(self.id2token_in_letters.get(input_letter, '<unk>'))
                input_letter = np.array([input_letter], dtype=np.int32)
                if j == 0 and length > 0:  # 下一词
                ...
                kc_output_prob = kc_interpreter.get_tensor(kc_output_details[1]['index'])
                kc_output_idx = kc_interpreter.get_tensor(kc_output_details[2]['index'])
                predict_top10, predict_top10_id = self.predictnext(kc_output_idx, logfile, curr_id, j, temp_y, curr)
                if j == 0 and length > 0:
                    #  判断是否预测成功
                    flag = False
                    #  碰到有空的情况
                    ...
                    if temp_y_deal.strip() == temp_y.strip() or (
                            len(temp_y_deal.strip()) - len(temp_y.strip())) == 1:  # 不带标点符号
                        if temp_y_deal in predict_top10[:3]:
                            save_count += len(temp_y_deal) - (j + 1)
                            logfile.write("correctly" + '\n')
                            break
                        else:
                            continue
        ...
        all_letter_count += word_count
        all_save_count += save_count

        result_next_save = "line input id :\t" + str(testfilein_id) + "\t总词数：\t" + str(
            count) + "\t预测成功词数：\t" + str(correct) + "\t总点击数：\t" + str(word_count) + "\t节省点击数：\t" + str(save_count)
        logfile.write(result_next_save.strip() + '\n')
        print(result_next_save)

        all_result = "line count:\t" + str(step) + "\tall count:\t" + str(
            all_count - step) + "\tall correct count:\t" + str(all_correct) + "\tall click count:\t" + str(
            all_letter_count) + "\tall save count:\t" + str(all_save_count)
