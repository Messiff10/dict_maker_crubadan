...
class LSTMModel(object):
    ...
    def testlite_lstm(self, sess, lm_tflitepath, kc_tflitepath, data, vocab_path):
        ...
        print("lm_input_details", lm_input_details)
        print("lm_output_details", lm_output_details)
        print("kc_input_details", kc_input_details)
        print("kc_output_details", kc_output_details)
        """Runs the model on the given data."""
        logfile = open('testlite_lstm.log', 'a+')
        testfilein = open(data, "r")
        sentence = "Mindy Olander"
        logfile.write(sentence.strip() + '\n')
        step = 0
        ...
        logfile.write("lm_output_state")
        logfile.write(lm_output_state)
        logfile.write("kc_output_state")
        logfile.write(kc_output_state)
        for i in range((length)):
            temp_y = labels_array[i]
            curr_id = self.word2id(lm_data_x[i])
            letters_ids = self.letters2ids(temp_y)
            # prediction_made+=1
            if i > 0:
                input_x = curr_id['id']
                print("out", curr_id['processed'])
                logfile.write("processed" + curr_id['processed'] + '\n')
                # print(input_x)
                ...
                # print('invoke!!!')
                print("lm_interpreter", lm_interpreter)
            ...
            for j in range(len(letters_ids)):
                ...
                curr += input_letter
                input_letter = np.array([curr], dtype=np.int32)
                if j == 0 and length > 0:
                    ...
                    print("kc_output_state", kc_output_state)
                    print("kc_output_prob", kc_output_prob)
                    print("kc_output_idx", kc_output_idx)
