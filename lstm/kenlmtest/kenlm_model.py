...
class kenlm_model():
    ...
    # 语料生成器，并且初步预处理语料
    @staticmethod
    def text_generator(texts, jieba_cut=False, count=0):
    ...
    def lm_train(self):
        ...
        executive_code = self.kenlm_model_path + 'lmplz -S {} -o {} --discount_fallback --skip_symbols {} < {} > {} '.format(
            self.memory,
            self.order,
            self.skip_symbols,
            self.corpus_file,
    ...
    def convert_format(self):
        ...
        ```
        ./kenlmtest/bin/build_binary weixin.arpa weixin.klm
        ```
    ...
    def read_ngrams(self):
        ...
        """
        # 数据读入
        # print("vocab_file"+self.vocab_file)
...
if __name__ == "__main__":
    ...
    # NLM模型训练文件生成
    # text_list = []
    text_list = open("/home/pubsrv/data/original/en_US/en_US_original", 'r', encoding='utf-8')
    text_list_out = open("/home/pubsrv/data/original/en_US/en_US_original_choose", 'w', encoding='utf-8')
    # import kenlm

    # model = kenlm.Model('/home/pubsrv/data/original/en_US/en_US_original2.bin')
    # # print(math.fabs(model.score(str('this is a sentence .'))))
    # for l in text_list:
    #     if math.fabs(model.score(str(l), bos=True, eos=True)) < 200:
    #         text_list_out.write(l.strip())
    #         text_list_out.write('\n')
    # text_list.close()

    # print(model.score('this is a sentence .', bos=True, eos=True))
    # # for l in text_list_file:
    ...
    #
    km.write_corpus(km.text_generator(text_list), km.corpus_file)  # 将语料转存为文本
    # #
    # # # NLM模型训练
    # status = km.lm_train()
    # print(status)
    ...
    #
    # # 模型读入与过滤
    # regex_en = re.compile('[^a-zA-Z0-9\']')
    # ngrams, total = km.read_ngrams()
    # print(ngrams, type(ngrams))
    # print(len(ngrams))
    # ngrams_2 = km.filter_ngrams(ngrams, total, min_pmi=[0, 1, 3,5,7])
    # file = open('/home/pubsrv/data/original/en_US/unigram', 'w', encoding='utf-8')
    # for l in range(0, len(ngrams)):
    #     if not re.search(regex_en, str(ngrams[l])):
    #         print(ngrams[l])
    #         result = str(ngrams[l]) + "\t" + str(total[l])
    ...
    # # n-grams读入
    # ngrams, total = km.read_ngrams()
    # ngrams_2 = km.filter_ngrams(ngrams, total, min_pmi=[0, 2, 4, 6,8])
    #
    # # 纠错
    # import pycorrector
    # # from pypinyin import lazy_pinyin, Style
    #
    # sentence = 'Have you heen to ?Good eats!'
    # idx_errors = regex.split(sentence.strip())
    # # print()
    # correct = []
    # for ide in idx_errors:
    #     print(ide, '=>', pycorrector.en_correct(ide))
